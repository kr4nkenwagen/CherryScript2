...
eval_post :: proc(obj: ^types.object_t) -> (ret_obj: ^types.object_t, code: types.exit_codes) {
	if obj.type != .JSON do return nil, .UNSUPPORTED_OBJECT_TYPE_IN_EVAL_POST
	...
	if header.type != .NULL {
		if header.type != .JSON do return nil, .HEADER_IS_NOT_JSON_IN_EVAL_POST
