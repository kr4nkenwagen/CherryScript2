...
eval_patch :: proc(obj: ^types.object_t) -> (ret_obj: ^types.object_t, code: types.exit_codes) {
	if obj.type != .JSON do return nil, .UNSUPPORTED_OBJECT_TYPE_IN_EVAL_PATCH
	...
	if header.type != .NULL {
		if header.type != .JSON do return nil, .HEADER_IS_NOT_JSON_IN_EVAL_PATCH
	...
	else do return nil, .BODY_NEEDS_TO_BE_STRING_OR_JSON_IN_EVAL_PATCH
