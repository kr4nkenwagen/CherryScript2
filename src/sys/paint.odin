...
paint_code :: proc(text: string, allocator := context.allocator) -> string {
	...
	for i < len(bytes) {
		ch := bytes[i]
		if ch == ' ' || ch == '\t' {
			emit(&p, "", string(bytes[i:i + 1]))
			i += 1
			continue
		}
		if ch == '"' || ch == '\'' {
			end, _ := scan_string(bytes, i)
			emit(&p, CLR_AMBER, string(bytes[i:end]))
			i = end
		} else if ch == '#' {
			...
			end := scan_word(bytes, i)
			word := string(bytes[i:end])
			if is_keyword(bytes, i, word) {
				emit(&p, CLR_TITLE, word)
			} else {
				emit(&p, "", word)
			}
			i = end
		} else if ch == '&' && i + 1 < len(bytes) && bytes[i + 1] == '&' {
			emit(&p, CLR_TITLE, "&&")
			i += 2
		} else if ch == '|' && i + 1 < len(bytes) && bytes[i + 1] == '|' {
			emit(&p, CLR_TITLE, "||")
			...
			emit(&p, CLR_TITLE, string(bytes[i:i + 1]))
			i += 1
		} else {
			emit(&p, "", string(bytes[i:i + 1]))
			i += 1
...
emit :: proc(p: ^paint_t, color: string, text: string) {
	if p.color != color {
		p.color = color
		if color == "" {
			strings.write_string(&p.b, CLR_RESET)
		} else {
